...
; =============================================================================
        ...
        ;      of remaining buffer
        ; v0  value replicated to all bytes (also referenced as q0 and d0)
        ; v1  value replicated to all bytes (also referenced as q1)
        ; v2  value replicated to all bytes
        ; v3  value replicated to all bytes

        mov     x10, x0             ; copy dst because we must return it
                                    ;  unchanged in x0
        dup     v0.16b, w1         ; form repeated byte pattern in a SIMD reg
        mov     v1.16b, v0.16b
        mov     v2.16b, v0.16b
        mov     v3.16b, v0.16b
...
#endif
        fmov    x1, d0              ; copy repeated byte pattern back to w1/x1
...
Memset_15
        st1     {v0.8b}, [x10], #8                        ; store 8
Memset_7
        st4     {v0.b,v1.b,v2.b,v3.b}[0], [x10], #4    ; store 4
        st3     {v0.b,v1.b,v2.b}[0], [x10]              ; store 3
        ret
...
Memset_12
        st1     {v0.8b}, [x10], #8                        ; store 8
Memset_4
        st4     {v0.b,v1.b,v2.b,v3.b}[0], [x10]        ; store 4
        ret
...
Memset_17_32
        ...
        st1     v0.16b, [x8]       ; store last 16 bytes of buffer, it will
                                    ;  overlap the store below for lengths
                                    ;  17-31
Memset_16
        st1     v0.16b, [x10]      ; store first 16 bytes of buffer
Memset_0
Done
...
ForceAlignment
        ...
        st1     v0.16b, [x10], x8  ; store 16, but add only length in x8 to
                                    ;  address
